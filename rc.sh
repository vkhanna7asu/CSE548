...
# Allow ping from client network to google DNS and back
$IPTABLES -A FORWARD -p icmp -o $Client_NET_IFACE -s 8.8.8.8 -j ACCEPT
$IPTABLES -A FORWARD -p icmp -s 10.0.2.2 -i $Client_NET_IFACE -d 8.8.8.8 -j ACCEPT
...
# Allow sync & return packets from established connections
$IPTABLES -A INPUT -p TCP --syn -j ACCEPT
$IPTABLES -A INPUT -p TCP -m state --state ESTABLISHED,RELATED -j ACCEPT

#Allow client & loppback to connect to we server on gateway
...
# ALlow traffic to http for gateway for OS updates
...
# ping from client CM's IP to DNS 8.8.8.8
$IPTABLES -A INPUT -p icmp -s 10.0.2.2 -d 8.8.8.8 -j ACCEPT

# REJECT ping to local host
...
# ALlow internet traffic for client to update OS
